...
class OpenAICompatibleSummarizer:
    ...
    def __call__(self, item: Item) -> DigestEntry:
        ...
        prompt = (
            "请仅返回 JSON，字段为 title、summary、why_important、category。"
            "title 是不超过28字的中文标题；summary 为60至110个中文字符，陈述已证实事实；"
            "why_important 为35至75个中文字符，说明具体影响；"
            "category 从以下选一：frontier(前沿模型)、agent(Agent与智能体)、vertical(垂类落地)、"
            "tools(开源与工具)、business(产品与商业)、policy(政策与治理)、research(研究前沿)。\n"
        ...
        try:
            ...
            return DigestEntry(result["title"].strip()[:28], item.source, item.url or "（无链接）",
