...
from .digest import DigestEntry, cutoff, local_summarize, render_digest, select_items
...
def run(config: Config, transport: Transport, *, dry_run: bool, now: datetime | None = None, summarizer: Summarizer = local_summarize) -> str:
    ...
    if len(selected) < config.minimum:
        raise RuntimeError(f"only {len(selected)} unique items in the last 24 hours; need {config.minimum}")
    text = render_digest([summarizer(item) for item in selected], generated_at=now)
